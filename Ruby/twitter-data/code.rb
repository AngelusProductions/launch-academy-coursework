...
twitter_data.each { |account| account.each { |user, info| if info["description"].nil? then puts user + ": N/A" + "\n\n" else puts user + ": "+ info["description"] + "\n\n" end } }
puts "\n"
twitter_data.each { |account| account.each { |user, info| puts user + ": " + info["number of followers"].to_s } }
puts "\n"
twitter_data.each { |account| account.each { |user, info| puts user + ": " + info["last twenty tweets"][0].length.to_s } }
puts "\n"
twitter_data.each { |account| account.each { |user, info| puts user + ": " + info["last twenty tweets"].join("").split("").length.to_s } }
puts "\n"
...
puts "#{most_popular} has the most followers."
puts "\n"
...
twitter_data.each do |account|
  account.each do |user, info|
    if info["number of friends"] > max_friends
      max_friends = info["number of friends"]
      most_connected = user
    end
  end
end
puts "#{most_connected} has the most friends."
puts "\n"
...
puts "#{most_tweety} has the most tweets."
puts "\n"

twitter_data.each { |account| account.each { |user, info| puts "#{user} has no description." if info["description"] == nil } }
puts "\n"

location = []
no_location = []
twitter_data.each { |account| account.each { |user, info| if info["location"].nil? then no_location << user else location << user end } }
puts "Has location:\n#{location.join("\n")}\n\n"
puts "No has location:\n#{no_location.join("\n")}"
